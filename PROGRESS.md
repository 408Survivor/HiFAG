...
### exp_21~exp_25 — A9 细+音频+x-attn（去粗分支，关键消融，2026-07-28，seeds 42~46）

| 配置 | test AUC (mean ± std) |
|------|----------------------|
| A9 细+音频（x-attn，无粗分支） | 0.7805 ± 0.0123 |

- 逐 seed：0.7783 / 0.7973 / 0.7630 / 0.7843 / 0.7796；valid AUC 0.800~0.832。
- 配对（同 seed）A8−A9 = 粗分支在完整模态下的净增量：
  +0.0155 / −0.0039 / +0.0276 / −0.0074 / −0.0030，**仅 2/5 为正，均值 +0.0058，
  t≈0.85，不显著**。
- 粗分支的价值目前主要体现在**稳定性**：A8 std 0.0088 vs A9 0.0123；
  均值上有小正贡献但不显著。
- 结论：并联（concat/x-attn）接入下粗图增量有限。**第二阶段层级交互
  （fine↔coarse 消息传递）成为放大粗图贡献的关键路径**，优先级上调。
- 论文叙事备选：sanity check 的强单变量信号 + A2 单独判别力 + 稳定性收益
  是粗图的立足点；均值增量需靠层级交互争取。
- 明细：`experiments/exp_21/` ~ `experiments/exp_25/`；汇总表见 `experiments/INDEX.md`。

### exp_26~exp_30 — A10 A8 + coarse→fine FiLM 层级交互（2026-07-28，seeds 42~46）❌ 阴性结果

| 配置 | test AUC (mean ± std) |
|------|----------------------|
| A10 A8+FiLM | 0.7824 ± 0.0066 |

- 逐 seed：0.7723 / 0.7819 / 0.7861 / 0.7818 / 0.7900；valid AUC 0.788~0.812。
- 实现：RegionGNN 节点嵌入按 (帧, landmark→region) gather 后 FiLM 调制细图
  几何 4 维输入，zero-init（起点 ≡ A8），参数 +520。
- 配对（同 seed）A10−A8：−0.0215/−0.0115/−0.0045/+0.0049/+0.0134，
  **2/5 为正，均值 −0.0039，t≈−0.63，不显著——FiLM 未放大粗图贡献**。
- 唯一亮点：std 0.0066 为三轮完整模态实验最低（A8 0.0088 / A9 0.0123），
  粗图（无论何种接法）持续带来稳定性收益。
- 教训记录（SFAF 教训 5）：zero-init 保证了不退化太多，但"粗图调制细图输入"
  这条路在 800 样本上没有可测的均值增量。可能的解释：粗描述子信号以时序 std
